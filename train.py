...
def main():
    ...
    checkpoint = ModelCheckpoint("model-{val_dice:.2f}.h5",ReduceLROnPlateau monitor="val_dice", verbose=1, save_best_only=True,
                                 save_weights_only=True, mode="max", period=1)
    stop = EarlyStopping(monitor="val_dice", patience=PATIENCE, mode="max")
    reduce_lr = ReduceLROnPlateau(monitor="val_dice", factor=0.2, patience=5, min_lr=1e-7, verbose=1, mode="max")
